...
/**
 * @brief Dump the given btree.
 *
 * @param b the #btree.
 * @param out a pointer to the @c FILE in which to dump the #btree.
 */
...
int btree_dump ( struct btree *b , FILE *out ) {
     ...
     * @param n the #btree_node over which to recurse
     */
    int btree_dump_rec ( struct btree_node *n ) {
    ...
    if ( b->first != NULL && btree_dump_rec( b->first ) < 0 )
        return btree_err;
    else
        return btree_err_ok;
