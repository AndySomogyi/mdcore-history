...
int dihedral_eval_set ( struct dihedral **d , int N , struct engine *e , int sid , double *epot_out ) {
    ...
    for ( did = 0 ; did < N ; did++ ) {
        ...
        ck = celllist[ pkd ]; ckd = ck->setID;
        cl = celllist[ pld ]; cld = cl->setID;
        ...
        if ( cphi < pot->a || cphi > pot->b ) {
            ...
            dlq[icount*3+1] = dxl[1];
            dlq[icount*3+2] = dxl[2];
            effi[icount] = ( cid == sid ) ? &pi->f[0] : dummy;
            effj[icount] = ( cjd == sid ) ? &pj->f[0] : dummy;
            effk[icount] = ( ckd == sid ) ? &pk->f[0] : dummy;
            effl[icount] = ( cld == sid ) ? &pl->f[0] : dummy;
            potq[icount] = pot;
            icount += 1;
