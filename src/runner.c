...
int runner_dopair_verlet2 ( struct runner *r , struct cell *cell_i , struct cell *cell_j , FPTYPE *pshift , struct cellpair *cp ) {
    ...
    dmaxdist = dscale * (cutoff + 2*eng->s.verlet_maxdx);
    ...
    /* No, genuine pair. */
        ...
        if ( s->verlet_rebuild || cp->pairs == NULL ) {
            ...
            for ( i = 0 ; i < count_j ; i++ ) {
                part_i = &( parts_j[i] );
                parts[count].ind = i;
                parts[count].d = 2 + dscale * ( (part_i->x[0]+pshift[0])*shift[0] + (part_i->x[1]+pshift[1])*shift[1] + (part_i->x[2]+pshift[2])*shift[2] );
                count += 1;
                }
        ...
        for ( i = 0 ; i < count_i ; i++ ) {
            ...
            /* loop over the left particles */
            for ( j = 0 ; j < count_j && parts[count_i+j].d - parts[i].d < dmaxdist ; j++ ) {

                /* get a handle on the second particle */
                part_j = &( parts_j[ parts[count_i+j].ind ] );
