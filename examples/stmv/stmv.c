...
int main ( int argc , char *argv[] ) {
    ...
    char *excl[] = { "OT" , "HT" };
    double L[] = { cutoff+0.05 , cutoff+0.05 , cutoff+0.05 };
    int devices[] = { 0 , 1 };
...
//        if ( e.flags & engine_flag_sets ) printf( " engine_flag_sets" );
    ...
    for ( step = 0 ; step < nr_steps ; step++ ) {
        ...
        if ( verbose && myrank == 0 ) {
            ...
            if ( ( res = MPI_Allreduce( MPI_IN_PLACE , e.timers , engine_timer_last , MPI_UNSIGNED_LONG , MPI_MAX , MPI_COMM_WORLD ) ) != MPI_SUCCESS ) {
                printf( "main[%i]: call to MPI_Allreduce failed with error %i.\n" , myrank , res );
                abort();
                }
        #endif
        ...
        #ifdef TIMER
            printf( "main[%i]: runner_timers = [ %.3f" , myrank , runner_timers[0]*itpms );
            for ( k = 1 ; k < runner_timer_count ; k++ )
                printf( " %.3f" , runner_timers[k]*itpms );
            printf( " ];\n" ); fflush( stdout );
